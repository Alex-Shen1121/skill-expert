...
**Skill 安装目录**：
中央技能库中当前已安装 Skill 对应的目录，其中的文件、子目录和符号链接共同构成该 Skill 的本地目录结构，包括隐藏项、缓存和管理文件。它与原始来源目录、Agent 或项目中的部署目录是不同的位置概念。
_避免使用_：Skill 有效内容、来源目录、部署目录

**Skill 入口文档**：
说明一个 Skill 的用途和使用方式的主文档，通常为 `SKILL.md`；它只是 Skill 安装目录中的一个文件，不代表该 Skill 的全部文件。
_避免使用_：完整 Skill、全部技能内容
...
**Agent 插件**：
由特定 Agent 作为一个整体提供和管理的能力包，保留共同的来源、版本、认证与部署关系；它与中央技能库中的独立 Skill 并列，不互相摊平或自动导入。
_避免使用_：普通 Skill、插件 Skill

**Agent 插件身份**：
由插件支持 Agent、Marketplace 和插件 ID 共同确定的安装实体；展示名称、版本或仓库地址相同都不会改变或合并这个身份。
_避免使用_：插件名称、仓库身份

**插件内置能力**：
只随所属 Agent 插件共同提供的 Skills、MCP、Hooks、连接器、浏览器扩展或自定义界面；它们不因此成为可独立部署的中央 Skill。
_避免使用_：中央技能、独立插件

**插件状态投影**：
从插件支持 Agent 的当前官方状态在同一次观察中生成的只读内存视图；它不是安装数据库、离线缓存或对插件状态的所有权记录。
_避免使用_：插件记录、插件缓存

**已安装插件全集**：
插件支持 Agent 的官方状态接口确认的全部已安装实体，包括本地与远程来源；产品界面是否另有隐藏规则不改变这个集合。
_避免使用_：Codex 可见插件、插件页数量

**部分插件状态投影**：
部分官方状态来源读取成功、其余来源读取失败时形成的可用视图；其中的成功状态仍可信，但总数明确不完整。
_避免使用_：完整插件清单、读取成功

**插件详情完整性**：
Agent 插件的说明与插件内置能力资料能够被可信读取的程度；图标或截图等视觉资源降级不改变完整性，它也不改变官方状态已经确认的安装事实。
_避免使用_：插件可用性、安装完整性

**插件认证策略**：
Agent 插件声明的可能需要认证的阶段，例如安装时或使用时；它不表示外部账户已经连接、授权或仍然有效。
_避免使用_：认证状态、连接状态

**插件安装状态**：
插件支持 Agent 当前明确报告的“已安装且启用”“已安装但停用”或“可安装”；停用属于已安装，展示资料不得覆盖该事实。
_避免使用_：插件详情状态、插件可用性

**插件更新可用性**：
插件支持 Agent 当前明确报告存在更新的状态；不得根据普通版本差异、Marketplace 同名条目或 manifest 推断。
_避免使用_：版本差异、推测更新

**插件支持 Agent**：
能够通过经过验证的官方状态适配器独立生成插件状态投影的 Agent；一个 Agent 失败不会改变其他 Agent 的投影。
_避免使用_：已启用 Agent、全部 Agent

**插件 CLI 可用性**：
插件支持 Agent 的官方 CLI 能否在当前应用环境中被定位、执行并提供兼容的插件目录命令；它与“当前没有插件”是不同事实。
_避免使用_：插件数量、CLI 安装状态
