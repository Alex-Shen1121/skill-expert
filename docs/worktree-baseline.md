...
诊断列出主工作目录、当前和全部 linked worktree、detached 状态、ahead/behind、staged/unstaged/untracked、远端刷新事实以及 recovery 记录：
...
把输出中的 `plan.id`、`plan.primaryWorktree`、旧本地 `main`、目标 `origin/main`、tracked 与 untracked 清单展示给用户。得到针对该计划的明确确认后，才可原样执行：
...
recovery 创建唯一的 `codex/local-main-recovery-<date>` 本地分支。tracked 变更会保存为最终内容快照，但不保留原先 staged 与 unstaged 的分界；untracked 内容只进入清单，不会被 add、stash、clean、移动或删除。本阶段不会移动 `main`，不会设置 upstream，也不会 push 或创建 PR。

## 显式同步本地 main

只有上一阶段产生且仍为 `verified` 的 recovery 才能进入同步。同步命令再次解析并 fetch 远端默认分支，确认计划目标、主工作目录、全部 worktree、进行中的 Git 操作、tracked/untracked 状态和 recovery 引用均未变化：
...
成功后必须同时满足：主工作目录位于 `main`；本地 `main`、`origin/main` 和 HEAD SHA 相同；ahead/behind 为 `0/0`；upstream 为 `origin/main`；recovery 仍可解析；既有 linked worktree 和 untracked 内容保持不变。
...
- untracked 路径与目标 checkout 冲突：工具安全停止，不会覆盖文件。由维护者另行决定内容去向，再从新的诊断计划开始。
