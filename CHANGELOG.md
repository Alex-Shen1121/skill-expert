...
### User-facing
- 差异目录新增“只看变化”，仅保留新增、删除和修改文件并展开父目录，可与搜索组合且不打断当前阅读；零变化、无匹配、目录不全和比较失败分别提示。
- 技能详情采用紧凑工作台，支持浏览当前安装目录的全部文件、隐藏项与空目录，并按需只读预览文本；来源与差异在访问时准备并共享同一版本，支持完整来源目录与跨版本文件定位，目录或文件变化会提示重新读取。
- 差异目录展示当前安装与来源的完整并集，按需阅读双栏全文，区分六种文件状态并保留完整执行位变化；片段计算达到工作量上限时明确提示，完整文本仍可查看。
- 文件目录支持按完整路径搜索、清除和定位；Markdown 可切换正文与保留元数据的原文，并安全阅读目录内关联文档，越界或缺失链接会明确提示。
...
### User-facing
- 设置页改为四个分类 Tab，合并更新与关于；Agent 页按同步技能和管理 Skills 区分用途，目录按需展开，管理能力使用复选框明确草稿与应用状态，保留现有功能和保存流程。
...
- 侧边栏新增“插件”入口，可在主从视图中组合使用已安装/可安装切换、本地搜索、Marketplace 筛选、键盘导航、安全 manifest 详情、手动刷新和可恢复的 Codex CLI 路径设置。
- 插件认证策略、详情完整性、内置能力、更新可用性和安装状态均使用明确文字、图标及适配主题的语义色表达，不会把认证策略误报为已连接或已授权。

### Developer & Governance
- 批次停止令牌已贯通远端修订解析、仓库缓存锁、Git 克隆和批量更新路径，并为 Git 远端查询增加有界超时与子进程回收。
- CLI 身份契约现在从权威模块校验管理 Skill 的独立仓库来源，并单独验证首页入口的可信安装接线，避免重构后误报。
- Agent 插件目录通过受控进程接缝读取同一次 Codex CLI 快照，以复合身份和严格 JSON 契约形成内存态投影，并安全补充逐条 manifest 资料，只暴露脱敏的结构化错误与技术详情。
- 新增独立 debug 验收入口，隔离应用自有状态与 Bundle 身份以验证真实 Tauri WebView；Windows 与 Linux 的不可晋级手工矩阵会在打包前运行受控进程 fixture。
